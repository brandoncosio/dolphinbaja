  ...
  return (
    ...
    <section className="relative z-10 w-full py-16 md:py-24 px-6 md:px-20 border-t border-white/5 overflow-hidden">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        >
          {/* Brillo sutil de fondo para iluminar los textos */}
          ...
          <p className="text-slate-200 text-sm sm:text-base md:text-lg leading-relaxed font-body drop-shadow-md max-w-xl mx-auto lg:mx-0">
            {content.desc}
          </p>
        </motion.div>

        {/* =========================================
            GRID DE TARJETAS (Auténtico Liquid Glass Apple-Style)
        ========================================= */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
          {valuesData.map((item, idx) => (
            <motion.div
              ...
              // 👇 EL CAMBIO A "APPLE GLASS": Usamos bg-white/5 y bg-white/10 al hacer hover
              className={`group relative p-6 md:p-8 rounded-[2rem] transition-all duration-500 md:hover:-translate-y-2 backdrop-blur-md overflow-hidden ${item.highlight
                  ? 'bg-gradient-to-br from-yellow-400/10 to-white/5 border border-yellow-400/30 shadow-[0_10px_30px_rgba(254,217,102,0.1)] hover:border-yellow-400/50 hover:shadow-[0_15px_40px_rgba(254,217,102,0.2)]'
                  : 'bg-white/5 border border-white/10 shadow-[0_8px_30px_rgba(0,0,0,0.2)] hover:bg-white/10 hover:border-cyan-400/30 hover:shadow-[0_15px_40px_rgba(102,216,227,0.15)]'
                }`}
            >
              {/* Luz interna en hover (Liquid Light) */}
              <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none ${item.highlight ? 'bg-gradient-to-tr from-yellow-400/10 to-transparent' : 'bg-gradient-to-tr from-cyan-400/5 to-transparent'
                }`} />
              ...
              <p className="relative z-10 text-xs md:text-sm text-slate-300 leading-relaxed font-body group-hover:text-slate-100 transition-colors">
                {item.desc}
              </p>
            </motion.div>
          ))}
